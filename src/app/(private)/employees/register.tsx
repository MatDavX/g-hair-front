    ...
    <Dialog open={enableClose} onOpenChange={setEnableClose}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Este formulário é utilizado para cadastro de novos funcionários no
            sistema.
          </DialogDescription>
        </DialogHeader>
...
export function InputForm({ setEnableClose }: Props) {
  ...
  return (
    <Form {...form}>
      <form id="form-user-register" onSubmit={form.handleSubmit(onSubmit)}>
        <Tabs defaultValue="account" className="w-[400px]">
          <TabsList className="grid w-full grid-cols-2">
            <TabsTrigger value="employee">Funcionário</TabsTrigger>
            <TabsTrigger value="address">Endereço</TabsTrigger>
          </TabsList>
          <TabsContent className="space-y-4" value="employee">
            <FormField
